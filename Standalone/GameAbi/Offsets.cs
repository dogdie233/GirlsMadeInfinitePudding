...
public static class Offsets
{
    ...
    // Texture2D's override of get_isReadable (Slot 11).  Reads a bool directly
    // off the native Texture struct via _Injected; never throws.  MUST be
    // checked before GetPixels32 or the runtime will raise an UnityException
    // that unwinds through our shellcode and crashes the game.
    public const ulong Fn_Texture2D_get_isReadable = 0x182170E40UL;

    // ---- Blit-to-readable-texture pipeline ----------------------------------
    // Used to grab pixels out of non-readable sprite textures by going through
    // a temporary RenderTexture:
    //   rt = RenderTexture.GetTemporary(w, h)
    //   Graphics.Blit(src, rt)
    //   prev = RenderTexture.get_active(); RenderTexture.set_active(rt)
    //   dst = new Texture2D(w, h)
    //   dst.ReadPixels(Rect(0,0,w,h), 0, 0, false)
    //   px = dst.GetPixels32()
    //   RenderTexture.set_active(prev); RenderTexture.ReleaseTemporary(rt)
    //   -> return px (Color32[]*)
    public const ulong Fn_Graphics_Blit_2 = 0x18211FF90UL; // (Texture, RenderTexture)
    public const ulong Fn_RenderTexture_GetTemporary_2 = 0x182163710UL; // (int, int)
    public const ulong Fn_RenderTexture_ReleaseTemporary = 0x1821642B0UL;
    public const ulong Fn_RenderTexture_get_active = 0x182162A10UL;
    public const ulong Fn_RenderTexture_set_active = 0x1821645F0UL;
    public const ulong Fn_Texture2D_ctor_ii = 0x18216FC50UL; // (this, int w, int h)

    public const ulong Fn_Texture2D_ReadPixels = 0x18216E110UL; // (this, Rect&, int, int, bool)
    // il2cpp_object_new(klass) is also exported by name and resolved via
    // RemoteExportResolver; we only need a Texture2D klass pointer which we
    // can grab from any live Texture2D's +0x00 (since every Texture2D instance
    // stores its klass there).  In practice the temp is made by calling the
    // ctor via il2cpp_object_new + ctor, so we also need the klass pointer.
    // To avoid hunting MethodInfo slots we reuse any live Texture2D we saw.
