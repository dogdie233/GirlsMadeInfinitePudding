...
public sealed class RemoteExportResolver
{
    ...
    private void Parse()
    {
        ...
        var e_lfanew = BitConverter.ToInt32(dos[0x3C..0x40]);
        ...
        // Magic (0x20b = PE32+)
        var magic = BitConverter.ToUInt16(nt.Slice(0x18, 2));
        if (magic != 0x20B)
            throw new InvalidOperationException($"Not PE32+ (magic 0x{magic:X}).");
        // DataDirectory[0] = Export
        var exportRva = BitConverter.ToInt32(nt.Slice(0x18 + 0x70 + 0, 4));
        var exportSize = BitConverter.ToInt32(nt.Slice(0x18 + 0x70 + 4, 4));
        ...
        var numberOfFunctions = BitConverter.ToUInt32(ed[0x14..0x18]);
        var numberOfNames = BitConverter.ToUInt32(ed[0x18..0x1C]);
        var addrOfFunctions = BitConverter.ToUInt32(ed[0x1C..0x20]);
        var addrOfNames = BitConverter.ToUInt32(ed[0x20..0x24]);
        var addrOfNameOrdinals = BitConverter.ToUInt32(ed[0x24..0x28]);
        ...
        {
            var name = ReadCString(_moduleBase + (int)names[i]);
            var ord = ordinals[i];
            if (ord >= funcs.Length) continue;
            var funcRva = funcs[ord];
            // Forwarded exports point into the export data-dir range; skip them.
            var forwarded = funcRva >= exportRva && funcRva < exportRva + exportSize;
